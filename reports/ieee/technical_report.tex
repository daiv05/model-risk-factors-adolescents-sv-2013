...
La Tabla~\ref{tab:ablation} presenta los resultados del análisis de ablación \textit{leave-one-group-out} partiendo de un F1 base de 0.498. Nota: este análisis usa validación cruzada de 5 folds sobre el dataset completo, por lo que los F1 no son directamente comparables con los del test set de la Tabla~\ref{tab:classification}.
...
\begin{enumerate}
  ...
  \item \textbf{El sistema propuesto puede operar como herramienta de vigilancia epidemiológica}: el clasificador de salud mental (AUC-ROC = 0.808, recall = 0.623) tiene capacidad suficiente para priorizar grupos de riesgo a escala departamental si se integra en una encuesta escolar anual.
\end{enumerate}
